...
def check_student_id(student_id: int) -> bool:
    return bool(db.session.execute(select(StudentModel.student_id).where(StudentModel.student_id == student_id)).all())


def check_course_id(course_id: int) -> bool:
    return bool(db.session.execute(select(CourseModel.course_id).where(CourseModel.course_id == course_id)).all())


def check_course_for_student(student_id: int, course_id: int) -> bool:
