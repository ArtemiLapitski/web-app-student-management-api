...
def test_1(db_setup, client, db_create_tables):
...
def test_2(db_setup, client, db_create_tables):
    response = client.put('students/2/courses/9')
    print(response)


def test_3(db_setup, client, db_create_tables):
    students_for_course = client.get('students', query_string={'course': 'Physics'})
    students_for_course = json.loads(students_for_course.data)
    assert students_for_course == students_for_physics_mocked


# def test_add_course(db_setup, client):
#     course = CourseModel(course_name="some_course")
#     session.add(course)
#     session.commit()
#
#
# def test_check_course_wasnt_added(db_setup, client):
